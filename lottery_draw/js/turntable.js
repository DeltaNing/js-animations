(function ($) {
    ...
    $.fn.WheelSurf = function (options) {
        var _default = {
            ...
            dots: {color: ['#fbf0a9', '#fbb936'],
                    count: 20 },
            disk: ['#ffb933', '#ffe8b5', '#ffb933', '#ffd57c', '#ffb933', '#ffe8b5', '#ffd57c', '#ffb933'],
            title: {
                color: '#5c1e08',
                font: '19px Arial'
            }
        ...
        return {
            init: function (angelTo) {
                ...
                // 平分角度
                var angel = (2 * Math.PI / 360) * (360 / num);// 相当于 2PI/num
                var startAngel = 2 * Math.PI / 360 * (-90) // 初始角度是 -PI/2
                var endAngel = 2 * Math.PI / 360 * (-90) + angel // 结束角度是 -PI/2 + 每个奖品的弧度
                ...
                ctx.arc(0, 0, 228, 0, 2 * Math.PI)
                ctx.stroke();

                // 装饰点
                var dotColor = _default.dots['color'], // 颜色
                dotCount = _default.dots['count']; // 装饰点的个数
                console.log(dotColor, dotCount)
                for (var i = 0; i < dotCount; i++) {
                    ...
                    startAngel += (2 * Math.PI / 360) * (360 / dotCount);
                ...
                for (var i = 0; i < num; i++) {
                    ...
                    ctx.arc(0, 0, 135, startAngel, endAngel)
                    ctx.stroke();
                    startAngel = endAngel
                    endAngel += angel
                }
                ...
                $.when(loadImg()).done(function (awardPic) {
                    ...
                    for (var i = 0; i < num; i++) {
                        ...
                        ctx.drawImage(awardPic[i], -38, -38 - 150, 76, 70);
                        ctx.font = _default.title.font;
                        ctx.fillStyle = _default.title.color
                        ctx.textAlign = "center";
                        ctx.fillText(awardTitle[i], 0, -230);
                        startAngel += angel
                        ctx.restore();
                    }
                })
